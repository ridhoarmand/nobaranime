  ...
  return (
    <div className="space-y-2">
      {resolutions.map((resolution) => {
        const links = dedupedDownloads[resolution];
        const resSize = links.find((l) => l.size)?.size;
          ...
          <div key={resolution} className="bg-zinc-900/50 rounded-lg border border-white/5 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {resSize && (
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-950/70 text-blue-300 border border-blue-800/40">
                    {resSize}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">
                  {links.length} provider{links.length !== 1 ? 's' : ''}
                </span>
              ...
              <div className="border-t border-white/5 p-3 flex flex-wrap gap-2 bg-black/30">
                {links.map((link) => (
                  <a
                    ...
                    className="px-3 py-1.5 rounded-lg bg-green-600/20 hover:bg-green-600/30 border border-green-600/30 text-xs md:text-sm text-green-400 hover:text-green-300 transition-colors flex items-center gap-1.5"
                  >
                    <span className="font-medium">{link.provider}</span>
                    {link.size && link.size !== resSize && (
                      <span className="text-[10px] text-green-300/80">({link.size})</span>
                    )}
                  </a>
                ))}
              </div>
